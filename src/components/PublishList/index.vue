  ...
  <div class="card card-custom">
    ...
    <div class="card-header border-0 py-5">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label font-weight-bolder text-dark">{{tableTitle ? tableTitle: '查询结果'}}</span>
            ...
            <tr class="text-left">
              ...
              <th  style="min-width: 80px;" @click="sortDataListByContent(item)" v-for="item in tableHead.slice(0,-1)" :key="item.name">
                ...
                <span class="svg-icon svg-icon-sm svg-icon-primary">
                  ...
                  <inline-svg src="media/svg/icons/Navigation/Down-2.svg" v-if="item.sortAble && item.currentSort == 1">
                  </inline-svg>
                  <inline-svg src="media/svg/icons/Navigation/Up-2.svg" v-if="item.sortAble && item.currentSort == 0">
                  </inline-svg>
                  <!--end::Svg Icon-->
                </span>
              </th>
        ...
        for (let oneRow of this.dataList) {
          ...
          for (let item of this.tableHead) {
            if (item.property.indexOf('.') > -1) {
              oneRowObj[item.property] = oneRow[item.property.split('.')[0]] ? oneRow[item.property.split('.')[0]][item.property.split('.')[1]] : '-'
            } else {
    ...
    methods: {
      ...
      sortDataListByContent(item) {
        ...
        if (item.currentSort != 0) {
          this.tableHead = this.tableHead.map(function (v) {
            if (v.name == item.name) {//更新升降序图标
              v.currentSort = 0;
            } else {
              v.currentSort = -1;
            }
