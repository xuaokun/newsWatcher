    ...
    <div class="row">
      ...
      <div class="col-lg-12">
        <div class="row">
          <div class="col-lg-8">
            <!--begin::Card-->
            <div class="card card-custom first-row" id="kt_todo_view">
              <!--begin::Header-->
              <div
                ...
                <div class="mb-3">
                  ...
                  <div class="cursor-pointer toggle-on" data-inbox="message">
                    ...
                    <div class="card-spacer-x pt-2 pb-5 toggle-off-item">
                      <!--begin::Text-->
                      <div class="mb-1">
                        <p v-for="item in eventInfo" :key="item.ID">
                          {{
                            item.content &&
                            item.content[0] &&
                      ...
                      >
                        <div v-for="oneEvent in eventInfo" :key="oneEvent.ID">
                          <router-link
                            v-if="oneEvent.relatedLaw"
                            :to="
                              '/fkgHome/lawDetail/' +
                                oneEvent.lawInfos[0]._source
                            "
        ...
        <div class="row">
          ...
          <div class=" col-lg-6">
            <div class="card card-custom history-timeline first-row">
              ...
              <!--begin::Body-->
              <div
                class="card-body pt-4 overflow-y-auto overflow-x-hidden"
              ...
              >
                ...
                <div class="timeline timeline-6 mt-3">
                  ...
                  >
                    ...
                    <!--begin::Text-->
                    <div
                      class="font-weight-bolder font-size-lg timeline-content pl-3"
                    >
                      <router-link :to="'/fkgHome/oneEventDetail/' + item.ID"
                        >{{ item.name }}
                      </router-link>
                    </div>
                    <!--end::Text-->
...
export default {
  ...
  props: ["eventId"],
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [{ title: "案例详情" }]);
  },
  created() {
    console.log(this.eventId);
    //获取案例title
    this.caseTitle =
      this.$route.query.info && this.$route.query.info.title
    ...
    this.axios
      .post("/api/sykg/query/events_infos/basic", { IDs: [this.eventId] })
      .then(data => {
    ...
    this.axios
      .post("/api/sykg/query/gremlin", {
        query: `V().hasLabel('Case').has('udfID','${this.eventId}').union(outE('caseObjectC').inV().bothE().not(hasLabel('BRANCH','INVEST')),out('CONTAINS').bothE())`
      })
      .then(data => {
        ...
        if (status == 0) {
          ...
          for (let item of message.nodes) {
            if (color_map[item.classType]) {
              ...
              color_map[item.classType] = colorDict[item.classType]
                ? colorDict[item.classType]
                : "#8ac4d0";
            }
          }
          ...
          for (let item of message.nodes) {
            ...
            if (
              (item.alias && item.alias.length > 10) ||
              item.name.length > 10
            ) {
              oneNode.text += "...";
            }
          ...
          for (let item of message.links) {
            if (item.relation == "eventObjectC" || item.relation == "PUNISHC") {
              data.links.push({
                from: item.target,
                to: item.source,
